...
const enable2FAStmt = db.prepare('UPDATE users SET is_2fa_enabled = 1, totp_secret = ? WHERE id = ?');
const disable2FAStmt = db.prepare('UPDATE users SET is_2fa_enabled = 0, totp_secret = NULL WHERE id = ?');

// Login token statements
const insertLoginTokenStmt = db.prepare('INSERT INTO login_tokens (token, user_id, expires_at) VALUES (?, ?, ?)');
const findLoginTokenStmt = db.prepare('SELECT user_id, expires_at FROM login_tokens WHERE token = ?');
const deleteLoginTokenStmt = db.prepare('DELETE FROM login_tokens WHERE token = ?');
const cleanExpiredTokensStmt = db.prepare("DELETE FROM login_tokens WHERE expires_at < datetime('now')");

// Login token attempts statements
const insertLoginTokenAttemptStmt = db.prepare('INSERT OR IGNORE INTO login_token_attempts (token, attempts) VALUES (?, 0)');
const getLoginTokenAttemptsStmt = db.prepare('SELECT attempts FROM login_token_attempts WHERE token = ?');
const incrementLoginTokenAttemptsStmt = db.prepare('UPDATE login_token_attempts SET attempts = attempts + 1 WHERE token = ?');
...
export function findUserById(id: number): DBUser | null {
  try {
    ...
    const error: any = new Error(`Error during user lookup by ID: ${((err as any)?.message) || String(err)}`);
...
export function is2FAEnabled(userId: number): boolean {
  try {
    ...
    const error: any = new Error(`Error checking 2FA status: ${((err as any)?.message) || String(err)}`);
...
export function getTotpSecret(userId: number): string | null {
  try {
    ...
    const error: any = new Error(`Error retrieving TOTP secret: ${((err as any)?.message) || String(err)}`);
...
export function createLoginToken(userId: number, expiresInSeconds: number = 120): string {
  try {
    ...
    const error: any = new Error(`Error creating login token: ${((err as any)?.message) || String(err)}`);
...
export function validateLoginToken(token: string): { userId: number; attempts: number } | null {
  try {
    const result = findLoginTokenStmt.get(token) as { user_id: number; expires_at: string } | undefined;
    ...
    const error: any = new Error(`Error validating login token: ${((err as any)?.message) || String(err)}`);
...
export function deleteLoginToken(token: string): void {
  try {
    ...
    const error: any = new Error(`Error deleting login token: ${((err as any)?.message) || String(err)}`);
...
export function incrementLoginTokenAttempts(token: string): void {
  try {
    ...
    const error: any = new Error(`Error incrementing login token attempts: ${((err as any)?.message) || String(err)}`);
...
export function getLoginTokenAttempts(token: string): number {
  try {
    ...
    const error: any = new Error(`Error getting login token attempts: ${((err as any)?.message) || String(err)}`);
...
export function cleanExpiredTokens(): void {
  try {
    ...
    const error: any = new Error(`Error cleaning expired tokens: ${((err as any)?.message) || String(err)}`);
